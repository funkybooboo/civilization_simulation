...
def setup_logger(mode='dev'):
    ...
    logger.add(sys.stdout, level=console_log_level, format="{time:YYYY-MM-DD at HH:mm:ss} | {level} | {message}")  # Log to console
    ...
    logger.add(os.path.join(log_dir, "error.log"), level="ERROR", format="{time:YYYY-MM-DD at HH:mm:ss} | {level} | {message}")
