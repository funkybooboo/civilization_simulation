...
class Construction(Work, ABC):
    def __init__(
        ...
        super().__init__(grid, location, width, height, char, max_worker_count=max_worker_count, max_work_count=max_work_count)
        self._required_wood: int = required_wood  # Total wood required for construction
        self._required_stone: int = required_stone  # Total stone required for construction
        self._delivered_wood: int = 0  # Total wood delivered so far
        self._delivered_stone: int = 0  # Total stone delivered so far
        self._current_completion_level: int = 0  # Tracks the current completion progress
        self._finished_completion_level: int = finished_completion_level  # Completion level when construction is finished
        self._max_worker_count: int = max_worker_count
        self._max_work_count: int = max_work_count  # Total work needed to complete the construction
    ...
    def deliver_wood(self, amount: int) -> None:
        ...
        if self._delivered_wood > self._required_wood:
            self._delivered_wood = self._required_wood  # Cap the delivery at the required amount
    ...
    def work_time_estimate(self) -> int:
        ...
        return (self._finished_completion_level - self._current_completion_level) * self._max_work_count
