...
class TaskState(State):
    ...
    def _get_average_task_count_with_predicate(self, task_predicate: callable) -> float:
        ...
        for person in self._people:
            total_count += sum(1 for task in person.get_scheduler().get_this_years_tasks() if task_predicate(task))
        average = total_count / len(self._people) if self._people else 0.0
        return average

    def _get_average_complete_task_count(self) -> float:
        return self._get_average_task_count_with_predicate(lambda task: task.is_finished())

    def _get_average_active_task_count(self) -> float:
        return self._get_average_task_count_with_predicate(lambda task: not task.is_finished())
    ...
    def _get_average_complete_eat_task_count(self) -> float:
        return self._get_average_task_count_with_predicate(lambda task: isinstance(task, Eat) and task.is_finished())
